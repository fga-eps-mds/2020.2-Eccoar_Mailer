...
describe("Test Send Email Controller", () => {
    ...
    test("Test if send emails is success", () => {
        ...
        jest.spyOn(QueueServices.prototype, 'addMailQueue').mockImplementation();
        jest.spyOn(QueueServices.prototype, 'emailQueueProcess').mockImplementation();
    ...
    test("Test if send emails is not a success", () => {
        ...
        expect(mResp.json).toHaveBeenCalledWith({'error': 'error to get email values'});
    });
